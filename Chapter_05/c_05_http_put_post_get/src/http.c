/* =====================================================================
   ...
   language governing permissions and limitations under the License.
    ...
    Added          do_http_put()
                   do_http_post()
                   do_http_get()
    Moved          RXMSG_SIZE to iot_monitor.h
                   doM2X = 1 to this file (from commands.cpp in original Avnet example)
======================================================================== */
...
//#define RXMSG_SIZE    250     // moved to iot_monitor.h
...
//=============================================================================
// Function:  do_http_put()
//
// This function provides a wrapper for the http_put() function. It takes
// the URL, Headers, and Data as parameters. With these, it builds the
// required http_info_t variable and calls the required number of
// http_add_field() call for http headers before calling http_put().
//=============================================================================
int do_http_put ( const char *url_param, int num_hdr, char **hdr, const char *data )
{
    int ret =0;
    int i;
    http_info_t put_req;
    ...
    // Call http_add_field for each header string that was passed
    if (num_hdr > 0) {
        for (i = 0; i <= (num_hdr - 1); i++) {
            printf("Header (%d) is: %s\n",  i, hdr[i]);
            put_req.header = http_add_field(put_req.header, hdr[i]);
        }
    }

    // Create field for data
    put_req.data_field = http_add_field(put_req.data_field, data);

    printf("Data: %s\n", data); //put_req.data_field);

    // Call http_put() function
    ret=http_put(&put_req, url_param);

    // Print response from http_put() function call
    if( put_req.rx_msglen >0) {
...
//
// This function provides a wrapper for the http_post() function. It takes
// the URL, Headers, and Data as parameters. With these, it builds the
// required http_info_t variable and calls the required number of
// http_add_field() call for http headers before calling http_post().
//=============================================================================
int do_http_post( const char *url, int num_hdr, char **hdr, const char *data, char * ret_buffer )
{
    int ret =0;
    int i;
    http_info_t post_req;
    ...
    if (num_hdr > 0) {
        for (i = 0; i <= (num_hdr - 1); i++) {
            printf("Header (%d) is: %s\n",  i, hdr[i]);
            post_req.header = http_add_field(post_req.header, hdr[i]);
        }
    }

    // Create field for data
    post_req.data_field = http_add_field(post_req.data_field, data);

    // Call http_post() function
    ret = http_post(&post_req, url);

    // Print response from http_post() function call
    if( post_req.rx_msglen > 0) {
        strcpy(ret_buffer, post_req.rx_msg);
    }
...
//
// This function provides a wrapper for the http_get() function. It takes
// the URL, Headers, and Data as parameters. With these, it builds the
// required http_info_t variable and calls the required number of
// http_add_field() call for http headers before calling http_post().
//=============================================================================
char *do_http_get( const char *url, int num_hdr, char **hdr, const char *data, char *response, int response_size )
{
    int ret =0;
    int i;
    http_info_t get_req;
    ...
    if (num_hdr > 0) {
        for (i = 0; i <= (num_hdr - 1); i++) {
            printf("Header (%d) is: %s\n",  i, hdr[i]);
            get_req.header = http_add_field(get_req.header, hdr[i]);
        }
    }

    // Create field for data
    get_req.data_field = http_add_field(get_req.data_field, data);

    // Call http_get() function
   do {
        ret = http_get(&get_req, url, response);
        }
    while( ret );
